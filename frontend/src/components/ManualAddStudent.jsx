...
export default function ManualAddStudent({ onSuccess, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!checkAuth.data?.user || !checkAuth.data.user.is_manager) {
        setMessage("❌ Vous devez être connecté en tant que manager pour ajouter un apprenant.");
        setLoading(false);
        return;
      }
      ...
      if (response.data.success) {
        ...
        setMessage(`❌ Erreur: ${response.data.error || 'Erreur lors de l\'ajout'}`);
      }
    } catch (error) {
      console.error("Erreur ajout apprenant:", error);
      const errorMsg = error.response?.data?.error || error.message || 'Erreur lors de l\'ajout';
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-green-900">Ajouter un apprenant</h3>
        {onClose && (
          <button
            onClick={onClose}
